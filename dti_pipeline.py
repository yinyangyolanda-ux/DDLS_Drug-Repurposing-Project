...
set_seed(SEED)
print(f"Random seed set to {SEED} for full pipeline determinism.") 
...
activities = activity_api.filter(target_chembl_id=target_chembl_id).filter(
    ...
    # DEFINITIVE CORRECTION: Specify acceptable assay types (B: Binding, F: Functional)
    assay_type__in=
).filter(
...
def generate_scaffold(smiles):
    ...
    # Uses RDKit's MurckoScaffoldSmiles function [1]
...
def scaffold_split_data(X_drugs, Y_labels, frac_test=0.2):
    ...
    # 2. Sort groups by size (largest first for deterministic, efficient splitting) [2]
    sorted_scaffolds = sorted(scaffold_groups.items(), key=lambda x: len(x[3]), reverse=True) 
    
    test_size_needed = int(len(df) * frac_test)
    test_smiles =
    
    # 3. Assign full scaffolds to the test set until the required size is met [4]
    ...
    # Validation check: shared scaffolds must be 0 for OOD test 
...
# Define the Hybrid Encoders [5, 6]
...
# DEFINITIVE CORRECTION: Setting hidden layer configuration for the fusion MLP.
...
# Calculate EF at 1% cutoff using ODDT [7, 8]
